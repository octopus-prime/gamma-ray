...
//		const scene::texture::texture_t& texture = hit->object->texture();
...
//		const float factor = (texture->shininess(point) + 2.f) / (8.f * M_PI);
		const float factor = (164.0 + 2.f);// / (8.f * M_PI);
//		const scene::color_t color = texture->pigment(point);
		const vector3_t color = {{1,0,0}};//texture->pigment(point);

		vector3_t I = boost::accumulate
		(
			_scene.lights(),
			(color & ambient) * 0.1,//texture->ambient(point),
			[&](vector3_t I, const scene::light::instance_t& light)
			{
				...
				if (shadow(ray_t(point, L, epsilon, LPl - epsilon)))
					return I;
				const vector3_t H = normalize(L - ray.direction());
//				const vector3_t d = std::max(0.0f, L * N) * texture->diffuse(point) * color;
				const vector3_t d = std::max(0.0f, L * N) * 0.7 * color;
//				const vector3_t s = factor * std::pow(std::max(0.0f, N * H), texture->shininess(point)) * texture->specular(point) * scene::WHITE;// * light.color();
				const vector3_t s = factor * std::pow(std::max(0.0f, N * H), factor) * 0.7 * vector3_t{{1,1,1}};// * light.color();
...
//			if (r > 0 && t == 0)
//			{
//				const vector3_t R = direction - 2.0f * (direction * N) * N;
//				I += (*this)(point, R, depth - 1) * r;
//			}
///*
//			const float t = texture->transparency(point);
//			if (t > 0.0f)
