...
(
	rt::scene::object::surface::primitive::text::basic_description_t,
	(std::string, text)
	(std::string, font)
	(float, extrusion)
)
...
extern template class string::parser<iterator_t, skipper::parser<iterator_t>>;
	...
	_filename()
{
	...
	_basic_description =
			qi::lit(NAME) > qi::lit('{')
			>
			qi::lit("text") > qi::lit('=') > _string
			>
			qi::lit("font") > qi::lit('=') > _filename
			>
			qi::lit("extrusion") > qi::lit('=') > qi::float_
			>
			qi::lit('}')
	;
}
